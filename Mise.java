...
public class Mise{
  ...
  //conversion avec les 20€ si client ;
  public int conversionValeur2(int valeur2,Mise m){
    int nb = 0 ;
  ...
  //conversion jeton en entier d'euros
  ...
  public int[] Miser(){
    if(nbOrange+nbVert+nbRouge+nbBlanc==0){
      System.out.println("Vous avez tout perdu") ;
      int[] tab = new int[4] ;
      ...
      if(Orange>nbOrange || Vert>nbVert || Rouge>nbRouge || Blanc>nbBlanc){
        System.out.println("Vous n'avez pas assez de jetons, tu all in !") ;
        tab[0] = nbOrange ;
        tab[1] = nbVert ;
        tab[2] = nbRouge ;
        tab[3] = nbBlanc ;
